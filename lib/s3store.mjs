...
export function createS3Store(config) {
  ...
  /**
   * 附件对象相对 key：files/{attachmentId}（无扩展名）。
   * 与用户文件名解耦（文件名仅存条目元数据 name）→ 防路径注入、防后缀伪装；
   * 附件不可变（attachmentId = uuid，创建走 If-None-Match），sha256 存元数据可校验。
   */
  function fileKeyOf(attachmentId) {
  ...
  /** GetObject → {body: string|Buffer, etag?}；404 → null。对可重试错误退避重试。 */
  async function getObject(key, { signal, binary = false } = {}) {
    return withRetry(async () => {
      const res = await request({ method: 'GET', path: `/${key}`, signal, notFoundIsNull: true });
      if (res === null) return null;
      // 附件二进制对象用 arrayBuffer 读（text() 会按 UTF-8 解码损坏字节）——附件面专用。
  ...
  return { getObject, putObject, deleteObject, headObject, listObjects, keyOf, fileKeyOf };
}
