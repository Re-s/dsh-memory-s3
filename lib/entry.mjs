...
export const TYPES = ['preference', 'project', 'decision', 'history', 'moment'];
...
export function validateEntry(entry) {
  ...
  const missing = [];
  for (const field of ['id', 'type', 'title', 'content', 'tags', 'importance', 'source', 'createdAt', 'updatedAt', 'recallCount', 'lastRecalled', 'workspaceKey', 'agentKey', 'locked']) {
  ...
  if (entry.subject !== undefined && (typeof entry.subject !== 'string' || entry.subject.trim() === '')) {
    return invalid('subject must be a non-empty string when present');
  }
  if (entry.timeline !== undefined && (typeof entry.timeline !== 'string' || entry.timeline.trim() === '')) {
    return invalid('timeline must be a non-empty string when present');
  }
  if (entry.links !== undefined) {
    if (!Array.isArray(entry.links)) {
      return invalid('links must be an array of entry id strings');
    }
    if (entry.links.some((l) => typeof l !== 'string' || l.trim() === '')) {
      return invalid('links entries must be non-empty strings');
    }
  }
  if (typeof entry.locked !== 'boolean') {
    return invalid('locked must be a boolean');
  }
...
export function normalizeEntry(input, { workspaceKey = '', agentKey = '', now = Date.now() } = {}) {
  ...
  if (input.subject !== undefined) {
    ...
    if (s !== '') entry.subject = s;
  }
  if (input.timeline !== undefined) {
    const t = String(input.timeline).trim();
    if (t !== '') entry.timeline = t;
  }
  if (input.links !== undefined) {
    if (!Array.isArray(input.links)) {
      throw invalid('links must be an array of entry id strings');
    }
    // 链接元素去重（保留首现顺序）、拒绝自引用与空串（容错但不静默吞错——杂数据拒收）。
    const seen = new Set();
    const cleaned = [];
    for (const raw of input.links) {
      const link = String(raw).trim();
      if (link === '' || link === entry.id || seen.has(link)) continue;
      seen.add(link);
      cleaned.push(link);
    }
    entry.links = cleaned;
  ...
  dbg(`normalize ok ${entry.id} type=${entry.type} locked=${entry.locked}`);
  return entry;
}
...
const JSON_FIELDS = [
  ...
  'createdAt', 'updatedAt', 'recallCount', 'lastRecalled', 'workspaceKey', 'agentKey', 'locked',
];

/** 附件序列化白名单（toJSON/fromJSON 共用；未知字段丢弃）。 */
const ATTACHMENT_JSON_FIELDS = ['id', 'name', 'mime', 'kind', 'size', 'sha256', 'objectKey', 'note', 'createdAt'];

/** 序列化为纯 JSON 对象（浅拷贝已知字段；可选字段存在才带上）。 */
export function toJSON(entry) {
  const out = {};
  for (const field of JSON_FIELDS) {
    out[field] = entry[field];
  }
  if (entry.subject !== undefined) out.subject = entry.subject;
  if (entry.timeline !== undefined) out.timeline = entry.timeline;
  if (Array.isArray(entry.links) && entry.links.length > 0) out.links = entry.links;
...
export function fromJSON(raw) {
  ...
  const entry = {
    ...
    locked: pick(raw, 'locked', (v) => typeof v === 'boolean'),
  };
  if (typeof raw.subject === 'string' && raw.subject.trim() !== '') entry.subject = raw.subject;
  if (typeof raw.timeline === 'string' && raw.timeline.trim() !== '') entry.timeline = raw.timeline;
  if (Array.isArray(raw.links)) {
    // 链接容错：非空字符串去重保留；坏项丢弃（悬空引用渲染层容错，此处只保形状）。
    const seen = new Set();
    const links = [];
    for (const l of raw.links) {
      if (typeof l !== 'string' || l.trim() === '' || seen.has(l)) continue;
      seen.add(l);
      links.push(l);
    }
