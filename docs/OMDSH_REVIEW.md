...
> 状态：v2.1 实现完成（QA 收尾）｜日期：2026-08-17（附件能力增补 2026-08-18；记忆模型 v2.1 增补 2026-08-18）
...
| lib/backlinks.mjs（反链索引：内存 Map + backlinks.json 0600 持久化 + 替换语义 + 悬空容错） | ✅ 实现完成 | **专测 test/backlinks.test.mjs 已建 10 例**（addForward 替换语义 / 自引用忽略 / allCounts / 0600 / 损坏容错 / removeForward 等） |
...
| index.mjs（插件入口：服务/工具/注入/审批；13 工具含附件三工具 + memory_s3_backlinks + 分层快照注入 + locked 合并保护） | ✅ 实现完成 | 57 例（mock ctx + mock fetch 集成；含 13 工具 / 分层注入 / locked / 反链挂钩断言） |
| 覆盖率（lib/ 全模块 + index.mjs） | ✅ 行 97.65% / 分支 90.26% / 函数 94.44% | 远超门 80/75/80（实测 `npm run coverage`，2026-08-18） |

> 🧪 **测试套件（2026-08-18 收尾态）**：`node --test` **212 例全绿**（entry 40 / backlinks 10 / index 57 / 其余 105）。含 v2.1 新增断言：moment 类型 / 四字段 toJSON（含 locked 计数）/ `test/backlinks.test.mjs` 10 例（addForward 替换语义 / 自引用忽略 / allCounts / 0600 / 损坏容错 / removeForward）/ 工具计数 13 / 分层注入 / locked 合并保护 / 反链挂钩 / 全部工具 render 回调。**先前挂起 3 例（removeForward / apply 注册九工具 / 全部 render 回调）已随实现语义对齐而全数通过。**

> **记忆模型 v2.1 状态（2026-08-18）**：实现完成（design 定稿 docs/MODEL.md）——五类型 + subject/timeline/links/locked 四字段全链路（lib/entry.mjs）、反链索引（lib/backlinks.mjs + index.mjs 挂钩）、`memory_s3_backlinks` 工具（13 工具）、locked 合并保护、分层快照注入（Bonds 40% → Moments → Facts + `→关联N` 标记）。**测试已收尾（212 全绿），本表复核通过。**
...
- [x] **记忆模型 v2.1 测试收尾**：✅ 已完成（212 例全绿）——moment 类型 / 四字段全链路 / locked 合并保护 / 反链索引（test/backlinks.test.mjs 10 例含 removeForward）/ 分层快照注入 / index 工具计数 13，本表复核通过
- [ ] **代码注释滞后（小幅残留，非阻断）**：`index.mjs` 内 `memory_s3_save` 工具**整体 description 已改五类型**（含 moment）；但 `type` **参数** description 仍写「Entry type: preference|project|decision|history」（参数 enum 已含 moment）——如实披露，文档升级不修改 index.mjs/types.d.ts
...
- 纯逻辑层：`node --test` **212 例全绿** + 覆盖率报告（all files 行 97.65% / 分支 90.26% / 函数 94.44%）
